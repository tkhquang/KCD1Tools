...
namespace TPVCamera::Presets
{
    namespace
    {
        ...
        constexpr BuiltinSpec k_builtins[] = {
            ...
            {k_builtin_sitting, "sitting"}, {k_builtin_kneel, "kneel"},
        };
    ...
    bool PresetStore::arrange_builtins()
    {
        ...
        // added by an update) is re-added from the embedded factory copy. The return value
    ...
    void PresetStore::load(const std::string &file_path)
    {
        ...
        // Ensure every built-in exists and order them first; a valid file missing a newer built-in (one
        // introduced by an update) has it re-added here without disturbing the user's own presets.
