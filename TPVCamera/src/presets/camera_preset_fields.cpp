...
namespace TPVCamera::Presets
{
    namespace
    {
        ...
        constexpr PresetField float_field(const char *key, const char *label, const char *tooltip, const char *group,
                                          float CameraPreset::*member, float min_value, float max_value,
                                          float fine_step) noexcept
        {
            return PresetField{key,    label,   tooltip,   group,     FieldType::Float,
                               member, nullptr, min_value, max_value, fine_step};
        }

        constexpr PresetField bool_field(const char *key, const char *label, const char *tooltip, const char *group,
                                         bool CameraPreset::*member) noexcept
        {
        ...
        constexpr PresetField k_fields[] = {
            ...
            float_field("follow_distance_min", "Follow Distance Min",
                        "Closest the zoom keys can pull the camera in, in meters.", "Framing",
                        &CameraPreset::follow_distance_min, 0.3f, 4.0f, 0.01f),
            float_field("follow_distance_max", "Follow Distance Max",
                        "Farthest the zoom keys can push the camera out, in meters.", "Framing",
                        &CameraPreset::follow_distance_max, 1.0f, 12.0f, 0.01f),
            float_field("zoom_step", "Zoom Step",
                        "How fast the camera moves while you hold a zoom key, in meters per second.", "Framing",
                        &CameraPreset::zoom_step, 0.1f, 10.0f, 0.01f),
            ...
            float_field("fov", "FOV",
                        ...
                        "STEALTH can narrow and COMBAT widen; it eases when the active preset changes.",
            ...
            float_field("orbit_pitch_max", "Orbit Pitch Max",
                        "Highest angle free-look can tilt up to, in degrees (positive looks up).", "Orbit",
                        &CameraPreset::orbit_pitch_max, 0.0f, 89.0f, 0.01f),
