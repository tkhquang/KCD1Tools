 ...
 *          specificity ties break by a fixed bit priority
 *          (Crouch > Combat > Mount > Lying > Sitting > Kneel), and an
 *          empty mask (DEFAULT) is the always-matching floor. This lets a user preset bound to e.g.
 *          {Combat, Crouch} win over the single-state built-ins when both are active, without anyone
...
namespace TPVCamera::Presets
{
    ...
    struct StateBindingTable
    {
        ...
        /// When set, @ref pinned overrides state selection (UI preview).
        bool has_pin = false;
        /// The editing preset to preview live while the panel pins it.
        CameraPreset pinned;
    };

    /**
     * @brief Parses a preset's bind_state token list into the GameState mask it auto-applies on.
     * @details Tokens are comma-separated, lowercase: the stance/state tokens "combat", "mount",
     *          "crouch" (alias "stealth"), "lying", "sitting", "kneel"; "minigame" and the per-minigame
     ...
     * @param bind_state The preset's bind_state string (e.g. "combat,crouch").
     ...
     * @details Inverse of parse_bind_mask, emitting in priority order the stance/state tokens (crouch,
     *          combat, mount, lying, sitting, kneel) plus a minigame token (the per-minigame token when a
     ...
     *          (Crouch > Combat > Mount > Lying > Sitting > Kneel); on identical masks the
     *          LATER entry wins (the '>=' tiebreak),
     ...
     */
    [[nodiscard]] int resolve_active_binding(std::uint32_t active_state, std::span<const std::uint32_t> masks) noexcept;
