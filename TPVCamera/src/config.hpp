...
namespace TPVCamera
{
    ...
    struct LiveSettings
    {
        ...
        std::atomic<bool> orbit_sprint_full_turn{
            true}; // sprint turn-and-run: when ON (default), face the FULL stick/key direction (no back-pedal, no
                   // +-90 clamp), so a backward push spins the body around to face the camera and runs toward it;
                   // when OFF a backward push back-pedals (faces away) and a sideways push faces at most sideways
        ...
        // Falls back to the RWI fan automatically when the engine sweep is unavailable, faults, or disagrees
        ...
        // Re-origin the player use/interaction cone onto the render camera + crosshair (third person) so the
        // use-target matches screen centre at all ranges. Live on KCD1 via the interactor selection-wrap in
        // interaction_hook.cpp (the selection's framework-view pose is transiently overwritten with the
        // camera + crosshair, origin slid to the eye projection so the range cap still measures from ~eye).
