...
namespace TPVCamera
{
    ...
    static void register_press_bindings()
    {
        auto add_press = [](std::string_view section, std::string_view ini_key, std::string_view log_name,
                            ...
                            bool consume_default = false)
        {
            s_binding_guards.push_back(DMK::Config::register_press_combo(section, ini_key, log_name, binding,
                                                                         std::move(on_press), default_combo));
            // Per-binding passthrough suppression (<Key>.Consume), registered right after the binding it targets
            // (so the name exists) and before load(). Default false = the trigger still reaches the game (current
            // behaviour); set true to mask the combo's GAMEPAD trigger button so a controller combo does not ALSO
            // fire the game action bound to that button. Inert for keyboard binds (DMK masks gamepad triggers
            // only). register_consume_flag copies the key + binding strings, so the temporaries below are safe.
            DMK::Config::register_consume_flag(section, std::string(ini_key) + ".Consume",
                                               std::string(log_name) + " Consume", binding, consume_default);
        };

        // Enter/exit third-person at runtime. Default: F3, or hold LB + press RB on a controller.
        add_press(
            "Settings", "ToggleViewKey", "Toggle View Key", "toggle_view",
            []
            {
        ...
        add_press(
            "Settings", "ForceFPVKey", "Force FPV Key", "force_fpv",
            []
            {
        ...
        add_press(
            "Settings", "ForceTPVKey", "Force TPV Key", "force_tpv",
            []
            {
                if (is_ui_blocking_input())
                    return;
        ...
        add_press(
            "Settings", "ToggleOverlayKey", "Toggle Overlay Key", "toggle_overlay", [] { Overlay::toggle(); }, "Home");
    }

    /**
     * @brief Registers the hold bindings (and the zoom triggers' INI consume flags) before load().
    ...
    static void register_hold_bindings()
    {
        ...
        DMK::Config::register_consume_flag("Camera", "ZoomOutKey.Consume", "Zoom Out Consume", k_zoom_out_binding,
                                           true);
        // The press-binding consume flags (incl. OrbitToggleKey.Consume, on by default for the LS/crouch
        // collision) are registered by add_press in register_press_bindings(), not here.
