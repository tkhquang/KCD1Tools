 ...
 *
 * @details Only compiled when KCD1_TPVCAMERA_DEV_BUILD is defined. In that build the
...
extern "C" __declspec(dllexport) bool Init() noexcept
{
    ...
    catch (...)
    {
        ...
        OutputDebugStringA("[KCD1_TPVCamera][DEV] Init() threw an exception; returning false\n");
        return false;
    }
}
...
extern "C" __declspec(dllexport) void Shutdown() noexcept
{
    ...
    catch (...)
    {
        ...
        OutputDebugStringA("[KCD1_TPVCamera][DEV] Shutdown() threw an exception; swallowed\n");
    }
}
