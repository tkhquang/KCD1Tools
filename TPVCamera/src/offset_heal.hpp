 ...
 * structs whose base is resolved live (the static CCryAction / the anchored global context). A game patch
 * that inserts or removes a struct member shifts every field after it, and a hardcoded offset then reads
 * the wrong slot. This unit recovers the shifted offsets at runtime with DetourModKit's reverse-RTTI
 * self-heal (rtti_dissect.hpp): each in-scope offset is keyed to the MSVC mangled name of the object its
 * slot points at, and the heal scans a small window around the nominal offset for the slot that still
 * resolves to that type. Only the offset is cached (a ptrdiff_t), never an absolute address, so the
 * recovered value stays valid across instances and sessions.
 *
 ...
 *
 * This is the KCD2 self-heal ported to KCD1's landmark set. KCD1's C_Player carries fewer RTTI-typed
 * members than KCD2: there is no embedded missile controller (so no missile field below) and no
 * HitDeathReactions member, so the look-controller bracket anchors on the actor model instead (see
 * offset_heal.cpp). The RTTI type names + nominal offsets all live in constants.hpp.
 */
...
namespace TPVCamera
{
     ...
     *          (both are valid offsets until the heal completes). KCD1 has no missile-controller member, so
     *          (unlike KCD2) there is no c_player_missile_controller field.
     */
    struct RuntimeOffsets
    {
    ...
    /**
     * @brief Heals the CCryAction -> CActionGame pointer offset once from the static CCryAction singleton.
     ...
     *          window scan keyed on that type. Latches on the first SUCCESS rather than the first call: very
     *          early in startup the slot can be null (CActionGame not constructed yet), and a call-count latch
     *          would freeze the nominal before the object exists; latching on success keeps retrying until it
     *          resolves. Render-thread only.
     * @param cry_action Live CCryAction singleton base address.
     */
     ...
     *          actor-model) and recovers the entity + look-controller offsets from a corroborated top-of-struct
     *          bracket; the animated-character pointer one hop further out is healed through the (healed)
     *          animated-human offset. The look controller has no RTTI of its own and the entity type (CEntity)
     *          is common, so both are pinned by the bracket's unique deep anchor (the actor model; KCD2 uses
     *          HitDeathReactions, which KCD1 lacks). The C_Player-direct group is deterministic once the caller
     *          has validated the vtable, so it latches after one pass; animChar latches separately once
     *          C_AnimatedHuman is live, so a frame where that pointer is briefly null retries on a later interval
     *          instead of being abandoned (no attempt cap). Caller must pass a C_Player whose vtable already
     *          matched C_PLAYER_RTTI_NAME. Render-thread only.
     ...
     *          insertion is recoverable here. Each member latches independently the first time its own heal
     *          resolves. Render-thread only.
