 ...
 *
 * Ported from the KCD2 TPVCamera (src/offset_heal.cpp) against the SAME DetourModKit reverse-RTTI self-heal
 * API. KCD1 1.9.7 carries fewer C_Player landmarks than KCD2: there is no embedded missile controller and no
 * HitDeathReactions member to anchor on, so those two landmarks are dropped. The corroborated top-of-struct
 * bracket that KCD2 builds from {entity, HitDeathReactions} is rebuilt here from {entity, actorModel}: the
 * actor model is the deepest C_Player member whose RTTI type is UNIQUE within the struct AND sits below the
 * no-RTTI look controller, so it takes over HitDeathReactions' role of pinning the delta and straddling the
 * look controller (see k_player_top_bracket). Everything else is the KCD2 logic verbatim.
 */
...
namespace TPVCamera
{
    ...
    namespace
    {
        ...
        // already-trusted concrete type), per the rtti_dissect guidance. base is filled at call time. Every KCD1
        // member here is a pointer-to-object (PointerToObject); unlike KCD2 there is no embedded missile controller,
        // so no CompleteObject landmark is needed.
        ...
        //     onto the wrong same-typed neighbour. Requiring entity and actorModel to agree on one delta rejects
        //     that decoy STRUCTURALLY, which is what lets the search window be widened safely.
        // KCD2 anchors this bracket on HitDeathReactions; KCD1 has no such member, so actorModel (C_ActorModel,
        // a type that is UNIQUE within C_Player) is the deep anchor instead. actorModel at C_PLAYER_ACTOR_MODEL_OFFSET
        // (0x908) sits BELOW the look controller (0x4C0), so the bracket [entity .. actorModel] still straddles it,
        // and the unique actorModel type pins the delta with no decoy. actorModel is ALSO healed independently
        // (heal_player_offsets) for its own slot; reusing the same landmark here only corroborates entity +
        // lookController and never writes the actorModel slot, so the two uses cannot disagree (both find the one
        // true delta).
        // Both anchors are required (the Landmark default), so the agreement is genuine. entity is the TOPMOST
        // anchor: a shift that moves it moves actorModel equally (the bracket then succeeds), and a shift below
        // entity leaves it at nominal (correct); a net insertion BETWEEN the anchors makes them disagree, so
        ...
        constexpr std::array<DMK::Rtti::Landmark, 2> k_player_top_bracket{{k_entity_lm, k_actormodel_lm}};
         ...
         *          the same structs (look pitch/yaw, stance, the aiming flag, hide-head, the CryEngine struct
    ...
    {
        // Latch on SUCCESS, never on call count or an attempt cap: the CActionGame pointer at CCryAction+0x78 is
        // null until the framework constructs it (cold boot, main menu, a slow load), so the heal must keep
        // retrying for as long as that takes and stop only once it actually resolves.
    ...
    void heal_player_offsets(std::uintptr_t c_player) noexcept
    {
        ...
        static std::atomic<bool> s_direct_done{false};   // animatedHuman + actorModel + the entity/lookController bracket
        ...
        if (!s_direct_done.load(std::memory_order_relaxed))
        {
            ...
            // instead: CEntity is a common type, so an independent scan would be decoy-prone.) Unlike KCD2 there
            // is no embedded missile controller to heal here. The C_Player vtable is already validated by the
            // caller, so this group is deterministic on a valid frame and latches after one pass; a miss is
            // logged at Debug (optional) since a recovered drift still warns loudly via heal_one's success path.
            ...
            // actorModel both agree on (see k_player_top_bracket). Each rides the delta on success; a non-uniform
            // shift fails the solve and both stay nominal (fail-closed).
