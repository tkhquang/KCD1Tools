...
namespace TPVCamera
{
    ...
    struct CameraState
    {
        ...
        // Camera settle-hold (render thread only). On KCD1 the eye-look is dragged 1:1 by the body, and when a
        // camera-relative run stops the engine keeps rotating the standing body for a short, intermittent window
        // (a stop / idle turn). The rig re-couples to the eye-look the instant orbit_moving clears, so it would
        // follow that turn and drift. While orbit_settle_hold is set, the world-stable (look-cancelling)
        // derivation is kept alive so the camera holds its world yaw, decoupled from the look, until the look
        // settles (then the orbit angle is baked and free-look re-couples with no jump). Started on the move-stop
        // edge, cleared on settle / move re-engage / orbit disengage / suppression. orbit_settle_look_prev is the
        // previous frame's eye-look yaw (radians) for slew detection; orbit_settle_elapsed is seconds since the
        // hold began (min/max bounds); orbit_settle_still counts consecutive ~still frames. Pairs with orbit_moving.
        bool orbit_settle_hold{false};
        float orbit_settle_look_prev{0.0f};
        float orbit_settle_elapsed{0.0f};
        int orbit_settle_still{0};
     ...
     *          eye, so the hook leaves the game untouched).
     *
