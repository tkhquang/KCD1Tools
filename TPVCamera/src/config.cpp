...
namespace TPVCamera
{
    ...
    void register_config_items()
    {
        ...
        DMK::Config::register_atomic<bool>("Orbit", "OrbitSprintFullTurn", "Orbit Sprint Full Turn",
                                           s.orbit_sprint_full_turn, true);
        ...
        // Minigame, and the per-minigame child tokens such as Dice/Reading/Lockpicking), parsed into bit masks
        // by parse_state_mask. The setters run on load and on every hot-reload, so editing a list in the INI
        // re-applies live; each setter overwrites its mask, so the parse is idempotent.
        // (KCD1 vs KCD2: there is no Cart stance on KCD1, so that token is not recognized; Aiming and the
        // lockpicking/hole-digging minigames ARE present. The minigame roster otherwise differs from KCD2.)
        ...
        DMK::Config::register_string(
            ...
            "Aiming,Dice,Reading,Alchemy,Sharpening,Backgammon,BookTranscription,Building");
        DMK::Config::register_string(
            "StateBehavior", "ForcedTPVState", "Forced TPV State", [&mask = s.forced_tpv_mask](const std::string &value)
            { mask.store(parse_state_mask(value), std::memory_order_relaxed); }, "Mount");
        DMK::Config::register_string(
            "StateBehavior", "OrbitExcludeState", "Orbit Exclude State",
            [&mask = s.orbit_exclude_mask](const std::string &value)
            { mask.store(parse_state_mask(value), std::memory_order_relaxed); },
            "Menu,Overlay,Combat,Mount,Lockpicking,Dice,Reading,Alchemy,Pickpocketing,Sharpening");
