...
3. Updates version in KCD1_TPVCamera_Readme.txt
...
README_TXT = BASE_DIR / "build" / "template" / "KCD1_TPVCamera_Readme.txt"
...
def update_readme_txt(version):
    """Update version in KCD1_TPVCamera_Readme.txt using string operations instead of regex."""
    ...
    try:
        ...
        print(f"Error updating KCD1_TPVCamera_Readme.txt: {str(e)}")
...
"""
    ...
    if not CHANGELOG_MD.exists():
        ...
        if "# Changelog" in content:
            ...
            if "All notable changes" in content:
                ...
                parts = content.split("All notable changes to the TPVCamera mod will be documented in this file.")
                if len(parts) > 1:
    ...
    header_end_pos = content.find("All notable changes to the TPVCamera mod will be documented in this file.")
    if header_end_pos != -1:
    ...
    if changelog_entry:
        ...
        if not version_in_links:
            version_links.append((version, f"https://github.com/tkhquang/KCD1Tools/releases/tag/TPVCamera-v{version}"))
...
def main():
    parser = argparse.ArgumentParser(description="TPVCamera Version Manager")
    ...
    if args.command == "bump":
        ...
        # Update KCD1_TPVCamera_Readme.txt with the new version
        update_readme_txt(version)
        if args.changelog:
            update_changelog(version, args.title, args.changelog)
        elif args.title:
            # A title with no changelog body writes no entry; warn rather than silently drop it.
            print("Warning: --title was given without --changelog, so no changelog entry was written.")
