...
namespace Mod
{
    ...
    void ToggleView()
    {
        SetCameraMode(GetCameraMode() == CAMERA_MODE_TPV ? CAMERA_MODE_FPV : CAMERA_MODE_TPV);
    }
    ...
    void Initialize()
    {
        ...
        DMKConfig::register_key_combo(
            ...
            [](const DMKKeyComboList& combos) { g_config.toggle_combos = combos; },
            "F3"
        );
        DMKConfig::register_key_combo(
            "Settings", "FPVKey", "Force FPV Keys",
            [](const DMKKeyComboList& combos) { g_config.fpv_combos = combos; },
            ""
        );
        DMKConfig::register_key_combo(
            "Settings", "TPVKey", "Force TPV Keys",
            [](const DMKKeyComboList& combos) { g_config.tpv_combos = combos; },
            ""
        );
        DMKConfig::register_string(
            "Settings", "LogLevel", "Log Level",
            [](const std::string& val) { g_config.log_level_str = val; },
            "INFO"
        );
        ...
        if (!cam_mgr)
        {
            logger.warning("GSC resolved but CameraManager not yet available; will retry on first input");
        }
        else
        {
